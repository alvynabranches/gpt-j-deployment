...
def index():
    logger.info(f"[lfqa] Status checked at {datetime.now()}")
    print(f"[lfqa] Status checked at {datetime.now()}")
    return jsonify({"status": "ok"}), 200


@app.route("/lfqa", methods=["GET"])
def lfqa():
    logger.info(f"Status of lfqa checked at {datetime.now()}")
    print(f"Status of lfqa checked at {datetime.now()}")
    return jsonify({"status": "ok"}), 200
