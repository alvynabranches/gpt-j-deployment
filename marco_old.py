...
def index():
    logger.info(f"[marco_old] Status checked at {datetime.now()}")
    print(f"[marco_old] Status checked at {datetime.now()}")
    return jsonify({"status": "ok"}), 200


@app.route("/marco_old", methods=["GET"])
def marco_old():
    logger.info(f"Status of marco_old checked at {datetime.now()}")
    print(f"Status of marco_old checked at {datetime.now()}")
    return jsonify({"status": "ok"}), 200
