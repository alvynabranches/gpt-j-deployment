...
def index():
    logger.info(f"[marco] Status checked at {datetime.now()}")
    print(f"[marco] Status checked at {datetime.now()}")
    return jsonify({"status": "ok"}), 200


@app.route("/marco", methods=["GET"])
def marco():
    logger.info(f"Status of marco checked at {datetime.now()}")
    print(f"Status of marco checked at {datetime.now()}")
    return jsonify({"status": "ok"}), 200
