...
def index():
    logger.info(f"[iag] Status checked at {datetime.now()}")
    print(f"[iag] Status checked at {datetime.now()}")
    return jsonify({"status": "ok"}), 200


@app.route("/iag", methods=["GET"])
def iag():
    logger.info(f"Status of iag checked at {datetime.now()}")
    print(f"Status of iag checked at {datetime.now()}")
    return jsonify({"status": "ok"}), 200
